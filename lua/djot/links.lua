...
local function visit_url(url)
	...
	if vim.startswith(url, "http://") or vim.startswith(url, "https://") or vim.startswith(url, "localhost") then
...
local function change_selection(selection, cb)
	local lines = vim.api.nvim_buf_get_text(0, selection[1], selection[2], selection[3], selection[4], {})
...
local function find_row_to_insert_link_def()
	...
	for i = line_count, 1, -1 do
		...
		if line and line ~= "" then
			local def = ts.find_node_from_cursor("link_reference_definition", { pos = { i - 1, 0 } })
